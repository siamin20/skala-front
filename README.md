...
>
> - 🔐 **로그인 · 마이페이지 · 아이디 저장** — `SHA-256` 해시 인증, 프로필 수정·탈퇴, 자동 로그인(아이디 저장)
> - 📝 **살아있는 회원가입 폼** — 아이디 **중복 확인** · 비밀번호 **강도 게이지**·규칙 체크 · **약관을 끝까지 읽어야 체크** · **제출 전 요약 모달**
> - 🧑‍🎨 **도트 캐릭터 아바타** — 이미지가 없으면 테마 색으로 **즉석 생성되는 인라인 SVG** 아바타
> - 🦖 **공룡 달리기 게임** — `<canvas>`로 점프·충돌 물리까지 구현한 러너 (최고점 저장)
> - 🗺 **색칠 세계지도** — 지도 라이브러리 없이 GeoJSON을 인라인 SVG로 (180여 개국)
> - 📍 **내 위치 날씨** — Geolocation + 역지오코딩으로 현재 지역 날씨
...
- **로그인** — 비밀번호는 **SHA-256** 해시로 저장·검증, `localStorage` 세션, 아이디 저장(자동 로그인)
- **마이페이지** — 프로필 수정·탈퇴, 비로그인 접근은 로그인 화면으로 가드
- **🧑‍🎨 도트 캐릭터 아바타** — 이미지가 없으면 테마 색으로 즉석 생성되는 인라인 SVG 아바타(헤더·마이페이지 공통)
