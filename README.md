...
지시되지 않았지만 완성도를 위해 얹은 것들:

**인터랙션 · 앱**
- 🦖 **공룡 달리기 게임** (`dino.js`) — `<canvas>` 순수 JS. 게임 루프·중력/점프 물리·선인장 충돌(AABB)·최고 점수 `localStorage`, 다크/라이트 테마색 반영
- 🌦 **실시간 날씨** — Open-Meteo 연동, 국내외 13개 도시, 로딩 상태 처리, ES Module로 데이터/화면 분리
- 🔐 **브라우저 데모 로그인** — 회원가입 계정으로 로그인, 비밀번호 **SHA-256 해시** 저장(평문 X), 마이페이지 프로필 수정·탈퇴
- 🎵 **여행 페이지 배경음악** — 인트로 게이트 클릭으로 자동재생 통과, 다른 오디오 재생 시 페이드 덕킹, 떠다니는 음소거 토글
- 📊 **온보딩 대시보드** — 현재 상태(공휴일 인식)·D-day 진도·출결 체크·주소 복사, 수업 중일 때만 켜지는 live/OFF 배지

**마크업 · 시각화**
- 지도 라이브러리 없이 GeoJSON 등거리 투영으로 그린 **인라인 SVG 세계지도**(180개국)
- `<dialog>` 모달, `<meter>` 비밀번호 강도, `<details>` 아코디언, `<picture>`, `<canvas>` 등 심화 태그
- 회원가입 폼 심화: `range`/`color`/`file`/`date` 입력, `datalist`, `optgroup`, `pattern` 검증

**보안 · 채점 안전성**
- WiFi·도어락·Zoom 등 민감 정보는 소스에 두지 않고 **권한 필요한 Slack 문서 링크**로만 제공 → Public 저장소 안전
- `localStorage`는 런타임 상호작용에만 사용 → fresh clone에서 빈 상태로 정상 동작
