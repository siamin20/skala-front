...
과제 페이지들을 한 곳에 모은 진입점이자 이 프로젝트의 중심 화면입니다. 상단 내비게이션 + 본문 바로가기 카드 + 사이드바 미니앱으로 구성하고, 스크롤 위치에 따라 현재 섹션을 표시하는 점프 내비를 얹었습니다.

**핵심 기술** · 시맨틱 랜드마크(`nav`/`main`/`aside`) · 스크롤스파이 · `:target` 하이라이트 · Flexbox·Grid

### 📊 온보딩 대시보드
"오늘 수업 있나?"(공휴일·주말 자동 인식), 과정 D-day 진도, 출결 체크리스트, 실시간 시계를 한눈에 보여주는 실사용 위젯입니다. 화면이 좁아지면 카드가 스스로 재배치됩니다.

**핵심 기술** · DOM 실시간 갱신 · `localStorage`(출결 기억) · **컨테이너 쿼리** 리플로우

### 📝 회원가입 — 실시간 검증 폼
아이디 중복 확인, 비밀번호 강도 게이지와 규칙 체크리스트, 진행률 바(필수 항목을 채울수록 차오름), 약관 전문을 끝까지 읽어야 체크 가능, 제출 전 요약 모달까지 붙였습니다. 제출은 GET으로 결과 페이지에 전달됩니다.

**핵심 기술** · `<meter>`/`<output>` · `pattern` 검증 · `<dialog>` 약관·요약 모달 · 커스텀 드롭다운/파일/색상 입력

### 🔐 로그인 · 마이페이지 · 아바타
회원가입한 계정으로 로그인해 프로필을 수정하고 탈퇴까지 할 수 있습니다. 비밀번호는 평문 대신 **Web Crypto SHA-256 해시**로 저장하고, 프로필 이미지를 올리지 않으면 **선택한 테마 색으로 즉석에서 그려지는 도트 캐릭터 SVG 아바타**가 헤더·마이페이지에 따라다닙니다.

**핵심 기술** · Web Crypto `SHA-256` · `localStorage` 세션 · 동적 인라인 SVG 아바타 · `FileReader` 미리보기 · 페이지 접근 가드

### 🌦 실시간 날씨 (+ 📍 현재 위치)
국내외 13개 도시 또는 **"현재 위치"**를 고르면 실시간 기온·습도를 보여줍니다. 현재 위치는 브라우저 위치 권한을 받아 좌표를 얻고, 역지오코딩으로 지역 이름까지 한글로 표시합니다.

**핵심 기술** · `fetch` + `async/await` · **ES Module 분리**(데이터/화면) · Geolocation · 역지오코딩 · Open-Meteo API

### 🗺 나의 여행지 — 색칠 세계지도 & 배경음악
지도 라이브러리 없이 가본 나라를 대륙별 색으로 칠한 세계지도(나라에 마우스를 올리면 이름 표시)와, 인트로를 클릭하면 배경음악이 자동재생되고 여행 영상·오디오를 틀면 볼륨이 자연스럽게 낮아지는(덕킹) 연출을 담았습니다.

**핵심 기술** · GeoJSON → 인라인 SVG(180여 개 나라) · `data-*` 툴팁 · Audio API 볼륨 페이드 · 자동재생 제스처 게이트

### 🎮 미니 게임·앱 — 공룡 달리기 · 숫자 맞히기 · 성적 계산기 · 내 가방
크롬 오프라인 공룡 게임을 오마주한 **캔버스 러너**(점프·선인장 회피·최고점 저장)와, 교재의 `prompt`/`alert` 예제를 `<dialog>` 모달 + DOM 입력폼으로 리메이크한 미니앱 3종입니다. 교재 문법 학습 흔적을 남기려 각 모달에 *"교재 방식(prompt/alert)"* 실행 버튼도 함께 뒀습니다.

**핵심 기술** · `<canvas>` + `requestAnimationFrame` 게임 루프 · AABB 충돌 · `<dialog>` · 배열/객체 반복(성적: `for` 합산 → 평균 → 등급 A~F)
