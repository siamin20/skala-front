...
> 과제 필수 요건을 모두 채운 위에, 완성도를 높이려고 직접 설계해 얹은 기능들입니다.
> 프레임워크·라이브러리 없이 순수 HTML · CSS · JavaScript로만 구현했습니다.

### 🏠 개인 포털 허브 — `index.html` (메인 화면)
과제 페이지들을 한 곳에 모은 진입점이자 이 프로젝트의 중심 화면입니다. 상단 내비게이션 + 본문 바로가기 카드 + 사이드바 미니앱으로 구성하고, 스크롤 위치에 따라 현재 섹션을 표시하는 점프 내비와 온보딩 대시보드를 얹었습니다.

**핵심 기술** · 시맨틱 랜드마크(`nav`/`main`/`aside`) · 스크롤스파이 · `:target` 하이라이트 · Flexbox·Grid 레이아웃

### 📊 온보딩 대시보드
"오늘 수업 있나?"(공휴일·주말 자동 인식), 과정 D-day 진도, 출결 체크리스트, 실시간 시계, 빠른 정보를 한눈에 보여주는 실사용 위젯입니다. 좁아지면 카드가 스스로 재배치됩니다.

**핵심 기술** · DOM 실시간 갱신 · `localStorage`(출결 기억) · **컨테이너 쿼리** 리플로우

### 📝 회원가입 — 실시간 검증 폼
단순 입력폼을 넘어, 아이디 중복 확인 · 비밀번호 강도 게이지와 규칙 체크리스트 · 비밀번호 일치 확인 · 진행률 바(필수 항목을 채울수록 차오름) · 약관 전문을 끝까지 읽어야 체크 가능 · 제출 전 요약 모달까지 붙였습니다. 제출은 GET으로 결과 페이지에 전달됩니다.

**핵심 기술** · `<meter>`/`<output>` · `pattern` 검증 · `<dialog>` 약관·요약 모달 · 커스텀 드롭다운/파일/색상 입력

### 🔐 로그인 · 마이페이지 (데모)
회원가입한 계정으로 로그인해 프로필을 확인·수정하고 탈퇴까지 할 수 있습니다. 비밀번호는 평문으로 저장하지 않고 **Web Crypto SHA-256 해시**로 저장하며, 로그인 상태에 따라 헤더가 "로그인 ↔ 마이페이지/로그아웃"으로 바뀝니다. 비로그인 상태로 마이페이지에 오면 로그인 화면으로 보냅니다.

**핵심 기술** · Web Crypto `SHA-256` · `localStorage` 세션·유저 저장소 · 페이지 접근 가드

### 🧑‍🎨 도트 캐릭터 아바타
프로필 이미지를 올리면 그 사진을, 안 올리면 **선택한 테마 색으로 즉석에서 그려지는 도트 캐릭터 SVG**를 아바타로 씁니다. 회원가입 미리보기·마이페이지·헤더 어디서나 같은 아바타가 따라다닙니다.

**핵심 기술** · 동적 인라인 SVG 생성 · `color-mix()` 색 틴트 · `FileReader` 업로드 미리보기

### 🌦 실시간 날씨 (+ 📍 현재 위치)
국내외 13개 도시 중 하나를 고르거나 **"현재 위치"**를 선택하면 실시간 기온·습도를 보여줍니다. 현재 위치는 브라우저 위치 권한을 받아 좌표를 얻고, 역지오코딩으로 지역 이름까지 한글로 표시합니다.

**핵심 기술** · `fetch` + `async/await` · **ES Module 분리**(데이터/화면) · Geolocation · 역지오코딩 · Open-Meteo API

### 🦖 공룡 달리기 게임
크롬 오프라인 공룡 게임을 오마주한 러너입니다. 점프로 선인장을 피하고, 최고 점수를 기억합니다.

**핵심 기술** · `<canvas>` · `requestAnimationFrame` 게임 루프 · 중력/점프 물리 · AABB 충돌 판정 · `localStorage` 최고점

### 🗺 색칠 세계지도
지도 라이브러리 없이, 가본 나라를 대륙별 색으로 칠한 세계지도입니다. GeoJSON을 직접 SVG 패스로 변환했고, 나라에 마우스를 올리면 이름이 뜹니다.

**핵심 기술** · GeoJSON → 인라인 SVG(전 세계 180여 개 나라) · `data-*` 속성 툴팁

### 🎵 여행 배경음악
`myTrip` 인트로를 클릭하면 배경음악이 자동재생되고(브라우저 자동재생 정책을 사용자 클릭으로 통과), 여행 영상·오디오를 틀면 배경음악이 자연스럽게 볼륨을 낮춥니다(덕킹).

**핵심 기술** · Audio API · 볼륨 페이드 · 사용자 제스처 게이트

### 🔢 미니앱 — 숫자 맞히기 · 성적 계산기 · 내 가방
교재의 `prompt`/`alert` 예제를 `<dialog>` 모달 + DOM 입력폼으로 리메이크했습니다. 대신 교재 문법 학습 흔적을 남기려고 각 모달에 *"교재 방식(prompt/alert)"* 실행 버튼도 함께 뒀습니다.

**핵심 기술** · `<dialog>` · 폼 이벤트 · 배열/객체 반복 · (성적 계산기는 `for` 합산 → 평균 → 등급 A~F)

### 🌗 다크 모드 · 반응형 · 접근성 (전체 공통)
헤더 토글 하나로 라이트↔다크가 바뀌고(OS 선호 자동 감지 + `localStorage` 기억, 깜빡임 방지), 786px 이하에서 세로 1열 + 햄버거 네비로 접힙니다. 스킵 링크·`aria-*`·`:focus-visible`·`prefers-reduced-motion`을 지켜 키보드·스크린리더도 배려했습니다.

**핵심 기술** · `data-theme` + CSS 변수 · `@media`/컨테이너 쿼리 · 시맨틱 마크업·ARIA

> 그 외 심화 마크업: `<meter>` 비밀번호 강도, `<details>` 아코디언, `<picture>` 반응형 이미지, `<time>`·`<abbr>`·`<blockquote>`, `datalist`·`optgroup`·`pattern` 폼 검증 등.
